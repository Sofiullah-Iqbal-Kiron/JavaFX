...
public class MathTextPropertyOptionsHolder {
    public GridPane theMathTextPropertyOptionsHolder = new FXMLLoader(getClass().getResource("MathTextPropertyOptionsHolder.fxml")).load();
    private static String methodNextLine = "\n\t\t"; // Escape sequence of next line in construct method.

    public MathTextPropertyOptionsHolder(String textName, FileWriter writer, TextArea manimTerminal) throws IOException {
        TextField mathtex = null;
        Slider fill_opacity = null;
        ColorPicker color = null;
        ComboBox<Integer> font_size = null, width = null;
        Button render = null;

        for (Node n : this.theMathTextPropertyOptionsHolder.getChildren()) {
            if (n instanceof Text) continue;
            else if (n instanceof TextField t) mathtex = t;
            else if (n instanceof Slider s) fill_opacity = s;
            else if (n instanceof ColorPicker cp) color = cp;
            else if (n instanceof ComboBox cb) {
                String cbID = cb.getId().strip();
                switch (cbID) {
                    case "font_size":
                        font_size = cb;
                        ...
                        font_size.getSelectionModel().select(21);
                        break;
                    case "width":
                        width = cb;
                        ObservableList<Integer> widths = FXCollections.observableArrayList(10, 20, 25, 30, 35, 40, 50, 60, 70, 100);
                        width.setItems(widths);
                        width.getSelectionModel().select(6);
                        break;
                }
            } else if (n instanceof Button b) render = b;
        }

        TextField finalMathText = mathtex;
        Slider finalFill_opacity = fill_opacity;
        ComboBox finalFont_size = font_size, finalWidth = width;

        final String[] colorInWebForm = new String[1];
        //        Change the color value of current math instance when any color has been picked up from the color_picker.
        ...
        render.setOnAction(e -> {
            ...
            try {
                writer.append(textName + " = MathTex(r'" + finalMathText.getText().strip() + "', fill_opacity=" + finalFill_opacity.getValue() + ", color='" + colorInWebForm[0] + "', font_size=" + finalFont_size.getSelectionModel().getSelectedItem().toString().strip() + ", width=" + finalWidth.getSelectionModel().getSelectedItem().toString().strip() + ")" + methodNextLine);
